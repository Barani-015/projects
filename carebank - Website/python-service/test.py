...
# # CHANGE THIS LINE - specify your model
# MODEL_NAME = "llama3.2:latest"  # 👈 CHANGE THIS
...
#         # Generate response from Ollama with specific model
...
#     print(f"🚀 Starting Python AI Service on port 5000...")
...
# def prepare_csv_context(csv_data: Dict[str, pd.DataFrame], max_rows: int = 100) -> str:
    """
    Prepare CSV data as context for Llama 3.2
    
    Args:
        csv_data: Dictionary of dataframes
        max_rows: Maximum rows to include per file (to avoid token limits)
    ...
    for filename, df in csv_data.items():
        ...
        # Add value counts for categorical columns (limit to top 5)
        ...
        for col in categorical_cols[:3]:  # Limit to 3 categorical columns
            if df[col].nunique() < 20:  # Only if not too many unique values
                context_parts.append(f"\nTop values in '{col}':")
                context_parts.append(df[col].value_counts().head(5).to_string())
    
    return "\n".join(context_parts)

# def format_transaction_query(user_id: str, question: str, csv_data: Dict[str, pd.DataFrame]) -> str:
#     """
#     Format a prompt for Llama 3.2 to answer transaction-related questions
#     """
#     csv_context = prepare_csv_context(csv_data)
    
#     prompt = f"""You are a helpful assistant for the CareBank project, specialized in analyzing transaction and financial data.

# USER ID: {user_id}

# Here is the transaction data from CSV files:

# {csv_context}

# Based ONLY on the CSV data provided above, please answer the following question:

# {question}

# Instructions for response:
# 1. ONLY use information from the CSV files provided
# 2. If the exact information isn't available, say "The data doesn't contain information about [topic]"
# 3. For transaction-related queries, provide specific amounts, dates, descriptions, and categories when available
# 4. Be concise but thorough in your response
# 5. Reference which CSV file the information came from
# 6. If doing calculations (sums, averages), show your work briefly
# 7. Format numbers with appropriate currency symbols
...
# def format_transaction_query(user_id: str, question: str, csv_data: Dict[str, pd.DataFrame]) -> str:
#     """
#     Format a prompt for Llama 3.2 to answer transaction-related questions - OPTIMIZED
#     """
#     csv_context = prepare_csv_context(csv_data, max_rows=5)  # Only 5 sample rows
    
#     prompt = f"""You are a helpful assistant for the CareBank project, specialized in analyzing transaction and financial data.

# USER ID: {user_id}

# Here is the transaction data from CSV files (showing summaries and first few rows only):

# {csv_context}

# Based on the data above, please answer: {question}

# Instructions:
# 1. Use the summary statistics and sample data provided
# 2. If exact information isn't available, say so honestly
# 3. Provide specific amounts and categories when possible
# 4. Be concise - answer in 2-3 sentences maximum
# 5. Format numbers with ₹ symbol
...
# ============= STAGE 2: LLAMA 3.2 INTERACTION =============

# def query_llama_with_csv(user_id: str, question: str) -> Dict[str, Any]:
    """
    Query Llama 3.2 with CSV context
    
    Args:
        user_id: User ID (folder name in uploadsCSVs)
        question: User's question about transactions
        
    Returns:
        Dictionary with response and metadata
    """
    try:
        # STAGE 1: Load CSV files for the user
        logger.info(f"📂 Loading CSV files for user: {user_id}")
        csv_data = load_user_csv_files(user_id)
        
        # STAGE 2: Prepare prompt with CSV context
        prompt = format_transaction_query(user_id, question, csv_data)
        
        # Check prompt length (Llama 3.2 has 128K context, but we'll log it)
        ...
        response = requests.post(
            ...
            timeout=120  # Longer timeout for processing CSV data
        )
        ...
        logger.error(f"❌ User folder error: {e}")
        return {
            "success": False,
            "response": f"User folder not found. User ID '{user_id}' doesn't have CSV files in uploadsCSVs folder.",
            "error": str(e),
            "expected_path": str(UPLOAD_CSV_FOLDER / user_id)
        }
    except ValueError as e:
        logger.error(f"❌ CSV reading error: {e}")
        return {
            "success": False,
            "response": f"No CSV files found for user '{user_id}'. Please upload CSV files to the uploadsCSVs/{user_id}/ folder.",
            "error": str(e)
        }
        ...
        return {
            "success": False,
            "response": "Request timed out. The CSV data might be too large. Try asking a more specific question.",
            "error": "Timeout"
        }
    except Exception as e:
    ...
    try:
        ...
        response = requests.post(
            ...
            json={
                ...
                "options": {
                    "temperature": 0.1,
                    "num_predict": 1024,  # Reduce max tokens
                    "top_k": 40,
                    "top_p": 0.9,
                }
            },
        ...
        return {
            "success": False,
            "response": "The analysis is taking too long. Please try a more specific question like 'Show my top 5 expenses' instead of 'Show all transactions'.",
            "error": "Timeout"
        }
    except Exception as e:
        logger.error(f"❌ Unexpected error: {e}")
...
# def prepare_csv_context(csv_data: Dict[str, pd.DataFrame], max_rows: int = 30) -> str:
    """Prepare CSV data with smart sampling"""
    context_parts = []
    
    for filename, df in csv_data.items():
        # Sample the data intelligently
        sampled_df = smart_sample_csv(df, max_rows)
        
        context_parts.append(f"\n{'='*50}")
        context_parts.append(f"📄 FILE: {filename}")
        context_parts.append(f"{'='*50}")
        context_parts.append(f"Columns: {', '.join(df.columns.tolist())}")
        context_parts.append(f"Total rows: {len(df)} (showing {len(sampled_df)} sampled rows)")
        context_parts.append(f"\nData sample:")
        context_parts.append(sampled_df.to_string())
        
        # Add summary statistics (this is lightweight)
        numeric_cols = df.select_dtypes(include=['number']).columns
        if len(numeric_cols) > 0:
            context_parts.append(f"\n📊 Summary:")
            context_parts.append(df[numeric_cols].describe().to_string())
...
@app.route('/chat', methods=['POST'])
def chat():
    """Original chat endpoint without CSV context"""
    try:
        data = request.get_json()
        prompt = data.get('prompt', '')
        ...
        # Simple chat without CSV context
        response = requests.post(
            f"{OLLAMA_URL}/api/generate",
            json={
                "model": MODEL_NAME,
